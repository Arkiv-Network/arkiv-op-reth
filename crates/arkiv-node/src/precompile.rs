//! Arkiv EntityDB-write custom precompile (POC).
//!
//! Installs a precompile at [`ARKIV_PRECOMPILE_ADDRESS`] that, on every
//! call, synchronously POSTs the calldata to EntityDB via JSON-RPC
//! (`arkiv_precompileWrite`) and returns the response's `stateRoot`
//! (32 bytes) as the precompile's return data.
//!
//! # Consensus assumption
//!
//! A precompile is part of the state-transition function. This module
//! only makes sense if EntityDB is itself deterministic — every node
//! executing the same transaction must hit an EntityDB that returns the
//! same `stateRoot`, byte-for-byte. The repo's `mock-entitydb.js`
//! always returns zero, which is fine for a single dev node. Multi-node
//! determinism is out of scope here.
//!
//! # Activation
//!
//! Active iff both `--arkiv.db-url` and `--arkiv.precompile` are set
//! (see `crate::cli` and `main.rs`). When inactive, the address has no
//! precompile installed and behaves like a regular EOA.
//!
//! # Wiring
//!
//! - [`entitydb_write_precompile`] — the [`DynPrecompile`] itself.
//! - [`ArkivOpEvmFactory`] — wraps the default OP `EvmFactory` and
//!   installs the precompile in `create_evm` /
//!   `create_evm_with_inspector` (both code paths must agree, otherwise
//!   tracing diverges from execution).
//! - [`ArkivOpEvmConfig`] — a local newtype around `OpEvmConfig` so we
...
//!   `ConfigureEvm` is a thin passthrough to the inner `OpEvmConfig`.
...
// Note: `revm` types are imported via `alloy_evm::revm` (which re-exports
// the matching `revm 38` line). The workspace also has `revm 36.0.0` in
// scope (used by `arkiv-genesis`); going through the alloy_evm re-export
// guarantees we get the version that matches `alloy-op-evm` and avoids
// silent type mismatches between the two `revm` versions.
use alloy_evm::{
...
use alloy_primitives::{Address, B256, Bytes, address, hex};
...
use reth_optimism_node::{
    ...
    // The node-local `OpPayloadBuilder<Txs = ()>` (vs the 3-generic one re-exported at the
    // crate root from `reth_optimism_payload_builder`).
    node::{
...
/// Fixed address. Outside the reserved precompile range (`0x01..=0x11`)
/// and distinct from the `EntityRegistry` predeploy at `0x44…0044`.
pub const ARKIV_PRECOMPILE_ADDRESS: Address =
    address!("0x000000000000000000000000000000000000aa01");

/// Flat gas cost. Order-of-magnitude DoS defence; not yet calibrated.
const GAS_COST: u64 = 5_000;

const PRECOMPILE_NAME: &str = "ARKIV_ENTITYDB_WRITE";
const ENTITYDB_METHOD: &str = "arkiv_precompileWrite";
...
/// Build the [`DynPrecompile`] bound to the given EntityDB client.
pub fn entitydb_write_precompile(client: Arc<EntityDbClient>) -> DynPrecompile {
    let id = PrecompileId::custom(PRECOMPILE_NAME);
    let call = move |input: PrecompileInput<'_>| -> PrecompileResult {
        if input.gas < GAS_COST {
            // Soft halt — preserves return-data semantics, doesn't abort the tx.
        ...
        let params = json!({
            "data": format!("0x{}", hex::encode(input.data)),
            "caller": input.caller,
            "value": input.value,
        });
        match client.rpc_call::<WriteResponse>(ENTITYDB_METHOD, params) {
            Ok(resp) => Ok(PrecompileOutput::new(
                GAS_COST,
                Bytes::copy_from_slice(resp.state_root.as_slice()),
                input.reservoir,
            )),
            Err(e) => {
                tracing::warn!(error = %e, "arkiv precompile: EntityDB call failed");
                // Fatal: aborts the tx. EntityDB unreachable is not a
                // recoverable EVM-level condition for this POC.
                Err(PrecompileError::Fatal(e.to_string()))
            }
        }
    };
    (id, call).into()
...
/// Wraps the default [`OpEvmFactory`] and (when a client is configured)
/// installs the EntityDB-write precompile on every fresh EVM instance.
...
impl ArkivOpEvmFactory {
    pub fn new(client: Option<Arc<EntityDbClient>>) -> Self {
        Self { inner: OpEvmFactory::default(), client }
    }
        ...
        E: Evm<Precompiles = PrecompilesMap>,
    {
        ...
        let precompile = entitydb_write_precompile(client.clone());
        evm.precompiles_mut()
...
impl ArkivOpEvmConfig {
    pub fn new(chain_spec: Arc<OpChainSpec>, client: Option<Arc<EntityDbClient>>) -> Self {
        let executor_factory = OpBlockExecutorFactory::new(
            OpRethReceiptBuilder::default(),
            chain_spec.clone(),
            ArkivOpEvmFactory::new(client),
        );
...
// ---------------------------------------------------------------------------
// ArkivOpNode — thin wrapper around `OpNode` that swaps the executor slot
// ---------------------------------------------------------------------------
//
...
impl ArkivOpNode {
    pub fn new(inner: OpNode, precompile_client: Option<Arc<EntityDbClient>>) -> Self {
        Self { inner, precompile_client }
    }
}
