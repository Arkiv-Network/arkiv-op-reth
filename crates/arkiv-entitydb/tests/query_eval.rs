...
//! `delete`, `transfer`) against a [`MemStore`], then parses
...
use arkiv_entitydb::test_utils::MemStore;
use arkiv_entitydb::{
...
#[track_caller]
fn ids(state: &mut MemStore, q: &str) -> Vec<u64> {
...
fn create_with_price(state: &mut MemStore, owner: Address, key: B256, price: u64) {
    create(
        state,
        owner,
        key,
...
fn range_historical_query() {
    ...
    // MemStore between writes. block-1 has price=100; block-2 has
